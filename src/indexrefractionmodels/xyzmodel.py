...
class XYZModel(AbstractIndexRefraction):
    def estimateIndexOfRefraction(self, layer: Layer) -> complex:
        ...
        if n_e == -1.0:
            ...
            igrfOutput = self.spacePhysicsModels.igrf.generatePointEstimate(layer=layer)
            ...
            cosTheta = dotAB / (
                np.linalg.norm(b_SEZ.data) * np.linalg.norm(ray_SEZ.data)
            )
            ...
            if self.transportMode is TransportMode.ORDINARY_MODE:
                num = (
                    ...
                    + 2 * eta_perp * eta_par
                    + sqrt(
                        b * b * sinTheta_sq * sinTheta_sq
                        + 4 * eta_cross * eta_cross * eta_par * eta_par * cosTheta_sq
                    )
                )
