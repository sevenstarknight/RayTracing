...
def generatePositionAndVector(currentState:RayState) -> ECEF:
    ...
    # ECEF location
    x_m, y_m, z_m = pyproj.transform(lla, ecef, lla_p1.lon_deg, lla_p1.lat_deg, lla_p1.altitude_m, radians=False)
    ...
    ecef_unit = ECEF(ecef_unitVector[0], ecef_unitVector[1], ecef_unitVector[2])
...
def computeSlantIntersections(ecef_m : ECEF, sVector_m : float, newAltitudes_m : list[float]) -> list[ECEF]:
    ...
    for newAltitude_m in newAltitudes_m:
        intersection_ECEF = computeNewIntersection(ecef_m, sVector_m, newAltitude_m)
