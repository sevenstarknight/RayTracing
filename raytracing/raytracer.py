...
class RayTracer():
    ...
    def execute(self, heights_m : list[float], indexN : list[complex], params : list[float]) -> list[RayState]:
        ...
        res = next(x for x, val in enumerate(heights_m) if val >= self.timeAndLocation.eventLocation_LLA.altitude_m)
        n_current = indexN[res]

        currentState = RayState(el, az, self.timeAndLocation.eventLocation_LLA, n_current)
        ...
        for idx in range(100):
            ...
            layerOutput = self.insideLayerOperations(currentState, heights_m, indexN, stateList)

            if(layerOutput.n_1 == None):
                logger.debug("Done with iterations, jump out of loop")
                stateList = layerOutput.stateList
                break
    ...
    def insideLayerOperations(self, currentState : RayState, heights_m : list[float], indexN : list[complex], stateList : list[RayState]) -> LayerOutput:
        ...
        if(max(heights_m) <= lla_p1.altitude_m):
            # exiting the loop
            layerOutput = LayerOutput(None, None, None, None, None, stateList=stateList)
        ...
        if(currentState.exitElevation_deg > 0):
            # going up
            res = next(x for x, val in enumerate(heights_m) if val > lla_p1.altitude_m)
        ...
        if(lla_p1.altitude_m in heights_m):
            ...
            if(newAltitude_m < lla_p1.altitude_m):
                ...
                logger.debug("up transitions")
        # ===================================================================
        lon_deg, lat_deg, alt_m = pyproj.transform(ecef, lla, ecef_p2.x_m, ecef_p2.y_m, ecef_p2.z_m, radians=False)
        lla_p2 = LLA(lat_deg, lon_deg, newAltitude_m) # force to altitude, loss because of approx.

        # estimate entry angle onto the curved layers
        entryAngle_deg = computeEntryAngle(currentState.exitElevation_deg, ecef_p1, ecef_p2, lla_p1, lla_p2)    

        layerOutput = LayerOutput(n_1, n_2, entryAngle_deg, newAltitude_m, lla_p2, stateList)
        return(layerOutput)
    ...
    def onTheEdgeOperations(self, currentState : RayState, layerOutput : LayerOutput) -> RayState:
        ...
        if(newAltitude_m == 0.0):
            ...
            currentState = RayState(90.0 - exitAngle_deg, currentState.exitAzimuth_deg, lla_p2, n_1)
            logger.debug("bounce off ground: flip direction of ray")
        else:
            f_2 = computeGeocentricRadius(lla_p2) + lla_p2.altitude_m 
            f_1 = computeGeocentricRadius(currentState.lla) + currentState.lla.altitude_m 
            ...
            if(argument > 1.0):
                ...
                if(exitAngle_deg >= 0):
                    # convert to elevation pointing up
                    currentState = RayState(90.0 - exitAngle_deg, currentState.exitAzimuth_deg, lla_p2, n_2)
                else:
                    # convert to elevation pointing down
                    currentState = RayState(-90.0 - exitAngle_deg, currentState.exitAzimuth_deg, lla_p2, n_2)
