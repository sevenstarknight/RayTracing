...
class TestIRIModel(unittest.TestCase):
    ...
    def test_iriModel(self):
        ...
        iriModel = IRI_Model(ionosphereState = ionosphereState, currentDateTime=currentDateTime)
        ...
        iriOutput = iriModel.generateSetEstimateFromRayState(rayPoints=slantPath)
        
        self.assertTrue(len(iriOutput) > 0)

        iono = iriOutput[0].iono

        print(iono['ne'][0].item())
